...
`ACTIONS sends data from the UI to the REDUCERS in the STORE. REDUCERS takes the data from the ACTIONS and updates the STORE according to the ACTIONS. The STORE is the single source of truth because it is where we get all of our immutable data for the entire application.`


- [x] What is the difference between Application state and Component state? When would be a good time to use one over the other?

`Application State is a 'global' state fromt the whole app and can be used throughout the entire app when needed.`
`Component State is state that is local to the certain component and it can be used when a certain component needs a certain state.`

- [x] Describe `redux-thunk`, what does it allow us to do? How does it change our `action-creators`?

`REDUX-THUNK allows us to run our redux app asynchronously. It's a part of middleware and looks to see if our actions returns objects or functions. If it's a function, then it'll be dispatched`
